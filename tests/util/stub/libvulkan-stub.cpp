...
VkBool32 vulkanSymbolWrapperInitLoader(void)
{
	...
	if (!vulkanModule)
	{
		vulkanModule = LoadLibraryA("vulkan-1");
	}

	if (vulkanModule)
	{
		auto gpa = reinterpret_cast<PFN_vkGetInstanceProcAddr>(GetProcAddress(vulkanModule, "vkGetInstanceProcAddr"));
		if (gpa)
			vulkanSymbolWrapperInit(gpa);
		else
			return VK_FALSE;
	}
	return vulkanModule != nullptr ? VK_TRUE : VK_FALSE;
#endif
}
